...
TokenParser.prototype = {
  ...
  parseToken: function (token) {
    ...
    if (lastState && lastState === _t.METHODOPEN && token.type !== _t.STRING) {
      self.state.pop();
    ...
    case _t.STRING:
      ...
      if (lastState === _t.METHODOPEN) {
        temp = self.state.pop();
        self.out.push(')');
        // Once off the string entry
    ...
    case _t.COLON:
      if (lastState !== _t.CURLYOPEN) {
        ...
        self.out.push('( '+ prevToken.match +' || _fn).call(');
        self.state.push(_t.METHODOPEN);
        self.escape = false;
        break;
      }
